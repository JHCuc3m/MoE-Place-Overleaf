...
Jane Yijun Sun conducted the literature review, provided high-level ideas on the
...
We investigate whether co-activation patterns in routing decisions can identify
experts that are safe to prune without significant quality degradation.
...
Our experiments on TinyMixtral-4x248M-MoE reveal a surprising finding: structural
metrics based on co-activation alone do not reliably predict pruning impact.
More strikingly, sensitivity analysis shows that one expert (Expert~2) is
\emph{harmful} across all 12 layers---disabling it \emph{improves} perplexity by
80--160 points.
These results suggest that co-activation analysis can identify not just redundant
experts but actively harmful ones, opening new directions for MoE model compression.
\end{abstract}
...
Our initial proposal~\citep{luo2025occult} focused on \emph{hardware-aware expert
...
Figure~\ref{fig:sensitivity} visualizes these results as a heatmap, clearly showing
the harmful Expert~2 column (blue) and the critical edge-layer experts (red).
...
\paragraph{Immediate (by project deadline).}
...
Develop a combined scoring function that incorporates sensitivity estimation
without requiring exhaustive per-expert evaluation.
Explore gradient-based importance approximations as a faster alternative to
...
Implement CAEP-Merge: merging co-activated experts instead of pruning.
